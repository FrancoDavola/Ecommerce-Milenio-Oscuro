...
res.render('index/cart', {
 ...
 store : function(req, res) { 
  ...
  let prodDuplicados =   noEliminados.find(function(prodDupli){
    ...
    producto.forEach(function(cucu){
        ...
        cucu.mercadoLibrePrecio = mercadoLibrePrecio
 ...
 probarPago : function(req, res){

 let paPagar =   noEliminados.map(function(dato){
        return {title : dato.name , unit_price : dato.mercadoLibrePrecio , quantity : Number(dato.cantidad)  }
    })
    ...
    let preference = {
        items: paPagar,
        back_urls: {
            success: "http://localhost:3000/cart/aprobado",
            failure: "http://localhost:3000/cart/aprobado",
            pending: "http://localhost:3000/cart/aprobado"
        }
      };
 ...
 aprobado : function(req, res){
    var configurations = {
        qs: {
          "id": req.query.collection_id
        }
      };
      
      mp.payment.search(configurations, function(error , response){
        response.response.results.forEach(function(estado){
            if(estado.status == "approved"){
                res.redirect("/cart/payment")
            } else {
                res.send(" el pago no se realizo con exito")
            }
        })
